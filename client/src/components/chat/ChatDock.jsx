...
export default function ChatDock() {
  ...
  const stopTyping = React.useCallback(() => {
    if (!socket || !currentRoom || !myId) return;
    socket.emit("chat:stopTyping", { room: currentRoom, userId: myId });
  }, [currentRoom, myId]);

  const markMessageError = React.useCallback((tempId) => {
    setMessages((prev) =>
      prev.map((msg) =>
        msg.clientId === tempId ? { ...msg, error: true, optimistic: false } : msg
      )
    );
  }, []);

  const replaceOptimistic = React.useCallback((tempId, newData) => {
    setMessages((prev) =>
      prev.map((msg) => (msg.clientId === tempId ? newData : msg))
    );
  }, []);
  ...
  const uploadFile = React.useCallback(
    async (file) => {
      ...
      try {
        ...
        form.append("clientId", tempId);
        form.append("userId", myId);
        form.append("userSub", myId);
        form.append("userName", displayName);
        if (user?.email) form.append("userEmail", user.email);
        if (isPrivateMode && selectedUserId) form.append("toUserId", selectedUserId);
    ...
    [currentRoom, isPrivateMode, markMessageError, myId, replaceOptimistic, selectedUserId, user]
  );
  ...
  React.useEffect(() => {
    ...
    const lastUserId =
      last?.user?.id || last?.user?._id || last?.user?.sub || last?.user?.email || null;
