...
function mapUserToFormSafe(api = {}) {
  ...
  return {
    ...
    profesion: getVal(
      api,
      ["profesion", "ocupacion", "persona.ocupacion"],
      ""
    ),
    lugarTrabajo: getVal(
      api,
...
function RoleBadges({ roles = [] }) {
  const uiNames = roles.map(
    (r) => ROLE_MAP_DB_TO_UI[r] || r
  );
  return (
...
function RoleSelect({ value = [], onChange }) {
  ...
  const selected = new Set(
    value.map((v) => ROLE_MAP_DB_TO_UI[v] || v)
  );
  const toggle = (uiName) => {
    const copy = new Set(selected);
    if (copy.has(uiName)) copy.delete(uiName);
    else copy.add(uiName);
    const dbList = Array.from(copy).map(
      (ui) => ROLE_MAP_UI_TO_DB[ui] || ui
    );
    onChange(dbList);
  };
  return (
...
function mapUserToForm(u = {}) {
  ...
  return {
    ...
    tipoDni: firstNonEmpty(
      u.tipoDni,
      p.tipoDni,
      "Identidad"
    ),
    dni: firstNonEmpty(u.dni, p.dni),
    estadoCivil: firstNonEmpty(
      u.estadoCivil,
      p.estadoCivil
    ),
    fechaNacimiento: (
      ...
      .toString()
      .slice(0, 10),
    paisNacimiento: firstNonEmpty(
      u.paisNacimiento,
      p.paisNacimiento
    ),
    ciudadNacimiento: firstNonEmpty(
      u.ciudadNacimiento,
      p.ciudadNacimiento
    ),
    municipioNacimiento: firstNonEmpty(
      u.municipioNacimiento,
      p.municipioNacimiento
    ),
    correoPersona: firstNonEmpty(
      u.correoPersona,
      u.email,
      p.correoPersona
    ),
    profesion: firstNonEmpty(
      u.profesion,
      p.profesion
    ),
    lugarTrabajo: firstNonEmpty(
      u.lugarTrabajo,
      p.lugarTrabajo
    ),
...
export default function UsersPage() {
  ...
  const showPwdRules =
    creds.password && creds.password.length > 0;
  ...
  function validate() {
    ...
    if (!form.nombreCompleto)
      e.nombreCompleto = "Nombre completo requerido";
    ...
    if (wantsPassword) {
      ...
      if (
        ...
        e.correoPersona =
          "Correo requerido/válido para enviar verificación.";
    }
    return e;
  }
  ...
  async function triggerVerification(userId, email) {
    ...
    if (typeof iamApi.sendVerificationEmail === "function") {
      ...
      throw new Error(
        "La API de verificación no está implementada en iamApi"
      );
    }
  }
    ...
    try {
      ...
      if (creds.sendVerification) {
        try {
          if (!savedId)
            throw new Error(
              "No se obtuvo el id del usuario guardado"
            );
          ...
          console.warn(
            "[UsersPage] verificación no enviada:",
            ev
          );
          alert(
      ...
      alert(
        "⚠️ Error al guardar: " +
          (e?.message || "Revisa la consola")
      );
      console.error(
        "[UsersPage] submit error:",
        e
      );
    } finally {
      setSubmitting(false);
  ...
  async function handleDelete(u) {
    const nombre =
      u?.nombreCompleto || u?.name || "este usuario";
    ...
    try {
      ...
      setItems(prev);
      alert(
        e?.message || "No se pudo eliminar el usuario"
      );
    }
  }
          ...
          <label className="space-y-1">
            ...
            {showPwdRules && (
              <ul className="text-xs opacity-80 mt-1 grid grid-cols-2 gap-x-4">
                <li>
                  {pwdR.length ? "✅" : "❌"} Mínimo 8
                  caracteres
                </li>
                <li>
                  {pwdR.upper ? "✅" : "❌"} Una mayúscula
                </li>
                <li>
                  {pwdR.lower ? "✅" : "❌"} Una minúscula
                </li>
                <li>
                  {pwdR.digit ? "✅" : "❌"} Un número
                </li>
              </ul>
            )}
          </label>
