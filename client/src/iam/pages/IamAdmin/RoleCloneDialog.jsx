...
export default function RoleCloneDialog({ open, onClose, roles, onCloned }) {
  ...
  const from = useMemo(
    () => roles.find((r) => r._id === fromId) || null,
    [fromId, roles]
  );
  ...
  async function onSubmit() {
    if (!from || !name.trim()) {
      setMsg(
        t("iam.roleClone.messages.selectRoleAndName", {
          defaultValue: "Selecciona el rol a clonar y un nombre.",
        })
      );
      return;
    }
    ...
    try {
      ...
      const created = await iamApi.createRoleWithPerms({
        code: newCode,
        name: name.trim(),
        description: t("iam.roleClone.messages.clonedFrom", {
          defaultValue: "Clonado de {{name}}",
          name: from.name,
        }),
      ...
      setMsg(
        e?.message ||
          t("iam.roleClone.messages.cloneError", {
            defaultValue: "No se pudo clonar el rol",
          })
      );
    } finally {
      setBusy(false);
      ...
      <div className="w-full max-w-lg rounded-2xl bg-white dark:bg-gray-900 shadow-xl overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-200 dark:border-gray-800">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">
            {t("iam.roleClone.title", {
              defaultValue: "Clonar rol",
            })}
          </h3>
        </div>
        ...
        <div className="p-5 space-y-4">
          <div className="space-y-1">
            <label className="text-sm text-gray-600 dark:text-gray-300">
              {t("iam.roleClone.fields.sourceRole", {
                defaultValue: "Rol origen",
              })}
            </label>
          ...
          <div className="space-y-1">
            <label className="text-sm text-gray-600 dark:text-gray-300">
              {t("iam.roleClone.fields.newRoleName", {
                defaultValue: "Nombre del nuevo rol",
              })}
            </label>
            ...
            <input
              ...
              placeholder={t("iam.roleClone.fields.newRoleNamePlaceholder", {
                defaultValue: "Ej. Operador Senior",
              })}
              className="w-full px-3 py-2 rounded border bg-white/70 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100"
            />
          </div>
          ...
          <div className="space-y-1">
            <label className="text-sm text-gray-600 dark:text-gray-300">
              {t("iam.roleClone.fields.codeOptional", {
                defaultValue: "Código (opcional)",
              })}
            </label>
            ...
            <input
              ...
              placeholder={t("iam.roleClone.fields.codePlaceholder", {
                defaultValue: "operador_senior",
              })}
          ...
          <label className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-700 dark:text-gray-300">
              {t("iam.roleClone.fields.includePermissions", {
                defaultValue: "Incluir permisos del rol origen",
              })}
            </span>
          </label>

          {msg && (
            <div className="text-sm text-red-600 dark:text-red-400">{msg}</div>
          )}
        </div>
        ...
        <div className="px-5 py-4 border-t border-gray-200 dark:border-gray-800 flex gap-2 justify-end">
          ...
          <button
            onClick={onSubmit}
            disabled={busy || !fromId || !name.trim()}
            className="px-3 py-2 rounded bg-indigo-600 text-white disabled:opacity-60"
          >
            {busy
              ? t("iam.roleClone.actions.cloning", {
                  defaultValue: "Clonando…",
                })
              : t("iam.roleClone.actions.clone", {
                  defaultValue: "Clonar",
                })}
          </button>
        </div>
