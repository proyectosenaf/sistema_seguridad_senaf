...
// Permite simular en DEV pero NO asumas admin fijo
function makeHeaders(token) {
  if (token) return { Authorization: `Bearer ${token}`, "Content-Type": "application/json" };

  const devEmail = import.meta.env.VITE_DEV_IAM_EMAIL || "dev@local";
  const devRoles = (import.meta.env.VITE_DEV_IAM_ROLES || "").trim(); // ej: "guardia,recepcion"
  const devPerms = (import.meta.env.VITE_DEV_IAM_PERMS || "").trim(); // ej: "rondas.read"

  const h = { "Content-Type": "application/json", "x-user-email": devEmail, "x-user-id": "dev-user" };
  if (devRoles) h["x-roles"] = devRoles;      // opcional
  if (devPerms) h["x-perms"] = devPerms;      // opcional
  return h;
}
...
async function req(path, { method="GET", body, token } = {}) {
  ...
  try {
    ...
    if (![404,405].includes(r1.status)) throw new Error((await r1.text().catch(()=> "")) || r1.statusText);
  } catch { /* fall back */ }
...
export const iamApi = {
  async me(token) {
    const h = makeHeaders(token);
    const c = [`${V1}/auth/me`, `${V1}/me`, `${LEGACY}/auth/me`, `${LEGACY}/me`];
    for (const u of c) try { const r = await fetch(u, opts({ headers:h })); if (r.ok) return j(r); } catch {}
