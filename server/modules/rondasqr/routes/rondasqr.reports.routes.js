...
function siteExpr()  { return { $ifNull: ["$siteName",  "-"] }; }
function roundExpr() { return { $ifNull: ["$roundName", "-"] }; }
function pointExpr() { return { $ifNull: ["$pointName", "-"] }; }
function qrExpr()    { return { $ifNull: ["$pointQr",   ""] }; }
...
async function buildAssignmentContexts(rows) {
  ...
  for (const r of rows) {
    ...
    if (!groups.has(k)) {
      groups.set(k, { date: r.date, guardId: r.guardId || "", roundId: r.roundId || "", siteId: r.siteId || "" });
    }
  }
  ...
  const assignments = await RqAssignment.find({
    ...
    guardId: { $in: guards },
    roundId: { $in: rounds },
  }).select({ date:1, guardId:1, roundId:1, siteId:1, startTime:1, endTime:1, planId:1, planSnap:1 }).lean();

  const ctx = new Map();
  // Cargar plan activo para donde haga falta
  const pairKeys = new Set([...groups.values()].map(g => `${g.siteId}|${g.roundId}`));
  ...
  const activePlans = await RqPlan.find({ siteId: { $in: sitesNeeded }, roundId: { $in: roundsNeeded }, active: true })
    .select({ siteId:1, roundId:1, points:1 }).lean();
  const planByPair = activePlans.reduce((m,p)=> (m[`${String(p.siteId)}|${String(p.roundId)}`]=p, m), {});

  const assByKey = assignments.reduce((m,a)=> (m[aggKey(a.date, a.guardId, String(a.roundId))]=a, m), {});
  for (const g of groups.values()) {
    const a = assByKey[aggKey(g.date, g.guardId, g.roundId)];
    let startTime, endTime, winMap = {};
    if (a) {
      startTime = a.startTime;
      endTime = a.endTime;
      const pts = (a.planSnap && a.planSnap.length) ? a.planSnap : (planByPair[`${g.siteId}|${g.roundId}`]?.points || []);
      winMap = {};
      for (const p of pts) {
...
function evalOnWindow(markISO, dateStr, pointId, ctxForKey) {
  ...
  if (!w || typeof w.startMin !== "number" || typeof w.endMin !== "number" || !startTime) {
    return { onWindow: null, windowStartMin: w?.startMin, windowEndMin: w?.endMin, toleranceMin: w?.toleranceMin ?? 0 };
  }
  ...
  return { onWindow: inWindow, windowStartMin: w.startMin, windowEndMin: w.endMin, toleranceMin: tol };
}
...
async function computeOmissions(q) {
  ...
  for (const m of marks) {
    ...
    if (!groups.has(k)) groups.set(k, { date: day, guardId: String(m.guardId||""), roundId: String(m.roundId||""), siteId: String(m.siteId||""), scanned: new Set() });
    if (m.pointId) groups.get(k).scanned.add(String(m.pointId));
  }

  const assignments = await RqAssignment.find({
    date: { $gte: dayjs(from).utc().format("YYYY-MM-DD"), $lte: dayjs(to).utc().format("YYYY-MM-DD") }
  }).select({ date:1, guardId:1, roundId:1, siteId:1, planId:1, planSnap:1 }).lean();

  for (const a of assignments) {
    const k = aggKey(a.date, a.guardId, String(a.roundId));
    if (!groups.has(k)) groups.set(k, { date: a.date, guardId: a.guardId, roundId: String(a.roundId), siteId: String(a.siteId || ""), scanned: new Set() });
  ...
  const activePlans = await RqPlan.find({ siteId: { $in: pairSiteIds }, roundId: { $in: pairRoundIds }, active: true })
    .select({ siteId:1, roundId:1, points:1 }).lean();
  const activePlanMap = activePlans.reduce((m,p) => (m[`${String(p.siteId)}|${String(p.roundId)}`]=p, m), {});
  ...
  for (const g of groups.values()) {
    ...
    if (g.assignment?.planSnap?.length) {
      ...
      if (plan?.points?.length) expected = plan.points.map(p => String(p.pointId)).filter(Boolean);
    }
  ...
  const points = allPointIds.size
    ? await RqPoint.find({ _id: { $in: [...allPointIds] } }).select({ name:1 }).lean()
    : [];
...
router.get("/reports/export/excel", async (req, res, next) => {
  try {
    ...
    Object.entries(byOfficer).forEach(([officer, v]) => ws1.addRow({ officer, ...v }));
    ...
    rowsDet.forEach(r => ws2.addRow({ ...r, onWindow: r.onWindow === null ? "" : (r.onWindow ? "Sí" : "No") }));
    ...
    res.setHeader("Content-Disposition", `attachment; filename="rondas_${Date.now()}.xlsx"`);
...
router.get("/reports/export/pdf", async (req, res, next) => {
  try {
    const rows = await fetchDetailedRows(req.query || {});
    const sum = rows.reduce((a,r)=> (a[r.officer] = (a[r.officer]||0)+1, a), {});
    const from = req.query.from || "-";
    const to   = req.query.to   || "-";

    const doc = new PDFDocument({ size: "A4", margin: 40 });
    res.setHeader("Content-Type", "application/pdf");
    res.setHeader("Content-Disposition", `attachment; filename="rondas_${Date.now()}.pdf"`);

    doc.fontSize(16).text("Informe de Rondas, Omisiones e Incidentes", { align: "center" });
    doc.moveDown(0.5);
    ...
    Object.entries(sum).forEach(([k,v]) => doc.fontSize(10).text(`• ${k}: ${v}`));
    doc.moveDown();

    doc.fontSize(12).text("Detalle");
    doc.moveDown(0.2);
    doc.fontSize(9).text("Fecha | Hora | Sitio | Ronda | Punto | Oficial | En ventana");
    doc.moveTo(doc.x, doc.y).lineTo(550, doc.y).stroke();

    rows.forEach(r => {
      const onw = r.onWindow === null ? "-" : (r.onWindow ? "Sí" : "No");
      const line = [r.date, r.time, r.siteName, r.roundName, r.pointName, r.officer, onw].join(" | ");
...
router.get("/reports/export/kml", async (req, res, next) => {
  try {
    ...
    const placemarks = items
      ...
      .map((i) => {
        const title = `${i.point || "-"} · ${i.site || "-"} · ${dayjs(i.at).format("YYYY-MM-DD HH:mm:ss")}`;
        const who = i.officer || "";
        return `
          <Placemark>
