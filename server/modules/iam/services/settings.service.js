...
export function getIamSettings() {
  return {
    otp: {
      ttlSeconds: Math.max(30, toInt(process.env.OTP_TTL_SECONDS, 300)), // ✅ evita TTL absurdos
      maxAttempts: Math.max(1, toInt(process.env.OTP_MAX_ATTEMPTS, 5)),
      resendCooldownSeconds: Math.max(
        0,
        toInt(process.env.OTP_RESEND_COOLDOWN_SECONDS, 30)
      ),
    },
    ...
    password: {
      expiresDays: Math.max(
        0,
        toInt(process.env.PASSWORD_EXPIRES_DAYS, 0)
      ), // 0 = sin expiración

      minLength: Math.max(
        6,
        toInt(process.env.PASSWORD_MIN_LENGTH, 8)
      ), // ✅ evita longitudes inválidas
    },
