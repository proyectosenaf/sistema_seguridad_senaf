...
export function makeNotifier({ io, mailer } = {}) {
  ...
  function emitInternalOnly(eventName, payload) {
    if (!io) return;
  ...
  async function panic({ userId, email, siteName, gps }) {
    ...
    if (io) {
      ...
      // compatibilidad con cualquier listener viejo
      emitInternalOnly("panic:new", payload);

      // nuevo evento alineado con server.js
      emitInternalOnly("alerta:nueva", payload);
    }
