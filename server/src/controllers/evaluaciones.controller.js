...
const estadoFromProm = (p) => (p >= 90 ? "Excelente" : p >= 80 ? "Satisfactorio" : "Requiere Mejora");
...
export const getMetricaPorEmpleado = async (req, res) => {
  try {
    ...
    const result = docs.map((e) => {
      ...
      const diasPuntuales = Math.round(clamp(e.puntualidad) * diasTotales / 100);
      const tardanzas = Math.max(0, diasTotales - diasPuntuales);

      const tareasCompletadas = Math.round(clamp(e.tareas) * tareasTotales / 100);
    ...
    res.status(500).json({ message: "Error al obtener métricas de evaluaciones" });
  }
};
...
export const getHistorial = async (req, res) => {
  try {
    ...
    const docs = await Evaluacion.find(filtro).sort({ fecha: -1, updatedAt: -1 }).lean();
    ...
    const items = docs.map((d) => {
      ...
      return {
        ...
        fecha: d.fecha || new Date(d.createdAt || new Date()).toISOString().slice(0, 10),
      };
    });
    ...
    res.status(500).json({ message: "Error al obtener historial de evaluaciones" });
  }
};
...
export const actualizar = async (req, res) => {
  try {
    ...
    const criterios = ["puntualidad", "tareas", "comunicacion", "iniciativa", "actitud"];
    if (criterios.some((k) => k in b)) {
      ...
      const promedio = clamp((vals.puntualidad + vals.tareas + vals.comunicacion + vals.iniciativa + vals.actitud) / 5);
      b.promedio = promedio;
      if (!b.estado) b.estado = estadoFromProm(promedio);
    }

    const upd = await Evaluacion.findByIdAndUpdate(id, b, { new: true });
    if (!upd) return res.status(404).json({ message: "Evaluación no encontrada" });
...
export const eliminar = async (req, res) => {
  try {
    ...
    if (!del) return res.status(404).json({ message: "Evaluación no encontrada" });
    res.json({ ok: true });
  } catch (err) {
...
export default { getMetricaPorEmpleado, getHistorial, crear, actualizar, eliminar };
